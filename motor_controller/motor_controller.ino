...
const float radius = 0.0325;                //Wheel radius, in m
float wheelbase = 0.194;                    //Wheelbase, in m
const float encoder_cpr = 160;              //Encoder ticks or counts per rotation
const float speed_to_pwm_ratio = 0.0036;    //Ratio to convert speed (in m/s) to PWM value. It was obtained by plotting the wheel speed in relation to the PWM motor command (the value is the slope of the linear function).
const float min_speed_cmd = 0.522;          //(min_speed_cmd/speed_to_pwm_ratio) is the minimum command value needed for the motor to start moving. This value was obtained by plotting the wheel speed in relation to the PWM motor command (the value is the constant of the linear function).
...
// PID Parameters
const double PID_left_param[] = { 0.0, 0.0, 0.1 };//{ 0.7, 0.2, 0 }; //Respectively Kp, Ki and Kd for left motor PID
const double PID_right_param[] = { 0.0, 0.0, 0.1 };//{ 0.7, 0.2, 0 }; //Respectively Kp, Ki and Kd for right motor PID
...
ros::Publisher twist_pub("/encoder_twist", &twist_msg);
...
ros::Publisher enc_ticks("/enc_ticks", &enc_msg);
...
void setup()
{
  ...
  nh.advertise(enc_ticks);
  nh.advertise(twist_pub);

  nh.getParam("/robot_base_L", &wheelbase);
  ...
  attachInterrupt(digitalPinToInterrupt(PIN_ENCOD_A_L)/*0*/, encoderLeftMotor, RISING);
  ...
  attachInterrupt(digitalPinToInterrupt(PIN_ENCOD_A_R)/*1*/, encoderRightMotor, RISING);
...
void loop()
{
  ...
  { // enter timed loop
    ...
    enc_msg.vector.x= pos_left;
    enc_msg.vector.y= pos_right;
    enc_ticks.publish(&enc_msg);
    pos_left = 0;
    pos_right = 0;
...
void publishSpeed(double time) {
  ...
  double omega = (speed_act_right - speed_act_left) / wheelbase;
  double V =(speed_act_right + speed_act_left) / 2.0;
